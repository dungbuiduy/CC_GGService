...
public class BrowserCommon_Extensions extends BrowserCommon {
	 ...
	 * @return @
	 */
	public boolean mediaCanBeDetected(ExcelFileController controllerExel, int rowInExcel, String excelLocation,
			HSSFSheet excelSheet, String siteName) {
		...
		if (waitForObjectPresent("pictures\\Browser_Savior_Button_DownloadFromDropDownlist.png", 5)) {
			...
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 2, "Y");
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 6, "Y");
	 ...
	 */
	public boolean buttonDownloadIsEnabled(ExcelFileController controllerExel, int rowInExcel, String excelLocation,
			HSSFSheet excelSheet, String siteName, String[] nameOfButtonDownload) {
		...
		if (waitForObjectPresent(nameOfButtonDownload[1], 15)) {
			...
			/*
			 * if (siteName.equalsIgnoreCase("xemphimso.com")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 * 
			 * if (siteName.equalsIgnoreCase("twitter.com")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[3], 25)){
			 * clickOn(nameOfButtonDownload[3]); } }
			 */

			/*
			 * if (siteName.equalsIgnoreCase("phim.in")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 */
			...
			/*
			 * if (siteName.equalsIgnoreCase("phim14.net")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]);
			 * 
			 * } }
			...
			/*
			 * if (siteName.equalsIgnoreCase("hayhaytv.vn")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 * 
			 * if (siteName.equalsIgnoreCase("hdviet.com")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 */
			/*
			 * if (siteName.equalsIgnoreCase("biphim.com")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 */

			/*
			 * if (siteName.equalsIgnoreCase("cohet.tv")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 */

			/*
			 * if (siteName.equalsIgnoreCase("vetv.vn")) { if
			 * (waitForObjectPresent(nameOfButtonDownload[2], 25)){
			 * clickOn(nameOfButtonDownload[2]); } }
			 */
		...
		// handle when checkpoint failed
			...
			TestLogger.warn(
					"Button download doesn't display when hovering on media content, download by cliking on download button on the Omibox bar ");
			...
			TestLogger.info("this must be download button on Omibox =>> OR " + nameOfButtonDownload[3] + "OR "
					+ nameOfButtonDownload[4]);
			switch (siteName) {
	 ...
	 * @return @
	 */
	public boolean buttonDetachIsDisplayed(ExcelFileController controllerExel, int rowInExcel, String excelLocation,
			HSSFSheet excelSheet, String siteName) {
		TestLogger.info("Check button detach display");
		boolean flag = false;
		if (waitForObjectPresent("pictures\\Browser_Savior_Button_PIN.png", 8)) {
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 8, "Y");
	 ...
	 * @return @
	 */
	public boolean checkButtonDetachWorksCorrectly(ExcelFileController controllerExel, int rowInExcel,
			String excelLocation, HSSFSheet resultSheet, HSSFSheet dataSheet, String siteName) {
		boolean flag = false;
		...
		if (waitForObjectPresent("pictures\\Browser_Savior_Button_PIPWindowRestore.png", 12)) {
			...
			case "phimmoi.net": {
				...
				if (waitForObjectPresent("pictures\\SaviorWeekly\\Website_PhimmoiButton_Pause.png", 5)) {
					controllerExel.writeToCell(excelLocation, resultSheet, "FinalResult", rowInExcel, 9, "Y");
					flag = true;
					TestLogger.info("--> PASSED");
			 ...
			 * controllerExel.writeToCell(excelLocation, resultSheet, "FinalResult",
			 * rowInExcel, 9, "Y"); flag = true; TestLogger.info("--> PASSED"); }
			 * 
			 * 
			 * break; }
			 */
	 ...
	 * @return
	 */
	public boolean detachedWindowRestoresCorrectly(ExcelFileController controllerExel, int rowInExcel,
		...
		if (waitforObjectNotexist("pictures\\Browser_Savior_Button_PIPWindowRestore.png", 12)) {
			...
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 10, "Y");
			flag = true;
		} else {
			TestLogger.warn("The restore button does not appears, please to check");
			String homePath = System.getProperty("user.home") + "/Desktop/screenShot/";
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 10, "Y");
			captureSnapshot(homePath, siteName, 1);
	 ...
	 * @param nameOfSong
	 */
	public boolean checkFileNameCanbeDetectCorrectly(ExcelFileController controllerExel, int rowInExcel,
			String excelLocation, HSSFSheet excelSheet, String nameOfSong, String siteName) {
		boolean flag = false;
		...
		if (flag) {
			TestLogger.info("--> PASSED");
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 3, "Y");
		} else {
			TestLogger.warn("Filename is Not Correct! Please check!");
			String homePath = System.getProperty("user.home") + "/Desktop/screenShot/";
			captureSnapshot(homePath, siteName, 1);
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 3, "N");
			// Note site to collect on report
			writeContainToTextFile("saviorReport\\filenameCanBeDetectedCorrectly.txt", siteName, true);
	 ...
	 * @return @
	 */
	public boolean mediaCanBePlayedWellOnLocalMachine(ExcelFileController controllerExel, int rowInExcel,
			String excelLocation, HSSFSheet excelSheet, String siteName) {
		...
		s.click();
		// waitForObjectPresent("pictures\\SaviorWeekly\\OSApp_MediaPlayer_Logo_VLC.png",
		// 2);
		...
		if (waitForObjectPresent("pictures\\OSApp_MediaPlayer_Button_PauseVLC.png", 3)) {
			...
			flag = true;
			controllerExel.writeToCell(excelLocation, excelSheet, "FinalResult", rowInExcel, 5, "Y");
			TestLogger.info("--> PASSED");
		} else {
	 ...
	 * @return @
	 */
	public boolean mediaCanBeDownloaded(ExcelFileController controllerExel, int rowInExcel, String excelLocation,
			HSSFSheet excelSheet, String siteName) {
	...
	public boolean checkSaviorIcon(String extensionStatus) {
		if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry.png", 10)) {
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png", -200);
		} else if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry_2.png", 10)) {
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry_2.png", -200);
		}
		// moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png", -200);
		sleep(0.5);
	...
	public boolean checkSaviorIconInIncognitoMode(String extensionStatus) {
		...
		if (extensionStatus.equalsIgnoreCase("Disapears"))
			return (waitforObjectNotexist("pictures\\Browser_Savior_Button_DownloadOnVideo_2.png", 2));
		else
			return waitForObjectPresent("pictures\\Browser_Savior_Button_DownloadOnVideo_2.png", 5);
	}

	/**
	 * acccess and play video on youtube (link:
	...
	public boolean accessWebAndPlayVideo(String webUrl) {
		...
		// if(waitForObjectPresent("pictures\\Website_youtube_Button_Registry.png",
		// 15)||waitForObjectPresent("pictures\\Website_youtube_Button_Registry_2.png",
		// 15)){
		// if(waitForObjectPresent("pictures\\Browser_Savior_Button_CloseWarningFonFirstTime.png",
		// 1)){
		...
		// sleep(5);
		// if(waitForObjectPresent("pictures\\Website_youtube_Button_Registry.png", 5))
		// {
		// moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png", -100);
		// }else
		// if(waitForObjectPresent("pictures\\Website_youtube_Button_Registry_2.png",
		// 5)) {
		// moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry_2.png",
		// -200);
		// }
		//// moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png",
		// -200);
		// }
		// break;
		// }
		...
		if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry.png", 15)) {
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png", -200);
		} else if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry_2.png", 15)) {
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry_2.png", -200);
		}
	...
	/**
	 * Download media content by click on download button and wait for download
	 * completed
	 * 
	 * @author huy.vu
	 */
	public boolean downloadMediaContentOnYoutube() {
		...
		if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry.png", 10)) {
			...
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry_2.png", -100);
		}

		if (System.getProperty("os.name").contains("Windows 7")) {
			if (waitForObjectPresent("pictures\\Browser_Savior_Button_DownloadOnVideo_win7.png", 10)) {
				clickOn("pictures\\Browser_Savior_Button_DownloadOnVideo_win7.png");
				sleep(3);
				// wait for download done
				isDownlodSuccess = waitForCocCocDownloadCompleted();
			}
		} else {
			if (waitForObjectPresent("pictures\\Browser_Savior_Button_DownloadOnVideo_2.png", 10)) {
				clickOn("pictures\\Browser_Savior_Button_DownloadOnVideo_2.png");
				sleep(3);
				// wait for download done
				isDownlodSuccess = waitForCocCocDownloadCompleted();
			}
	...
	public boolean waitForCocCocDownloadCompleted() {
		...
		if (waitForObjectPresent("pictures\\Browser_Torrent_Button_PauseDownload.png", 15)) {
			// if
			// (waitforObjectNotexist("pictures\\Browser_Torrent_Button_PauseDownload.png",
			// 360)) {
			if (waitforObjectNotexist("pictures\\Browser_Downloader_Button_PlayFileDownloaded.png", 360)) {
				TestLogger.info("Download  done");
				isFileDownloading = true;
			}

		} else if (waitForObjectPresent("pictures\\Browser_Downloader_Button_PlayFileDownloaded.png", 15)) {
			TestLogger.info("Download  done");
			isFileDownloading = true;
		} else {
			TestLogger.info("Download  failed, please check");
			String homePath = System.getProperty("user.home") + "/Desktop/screenShot/";
			captureSnapshot(homePath, "DownloadVideoFailed", 2);
		}
	 ...
	 * 
	 * @param videoQuality
	 *            quality of video want to download: FullHD, HD, Standard, Medium,
	 *            Small, mobile
	 * @author huy.vu
	 * @return String
	 * 
	...
	public void selectQualityVideoToDownload(String videoQuality) {
		...
		if (System.getProperty("os.name").contains("Windows 7")) {
			imageName = "saviorExtension_DownloadWith" + videoQuality + "_win7.png";
		} else {
			imageName = "saviorExtension_DownloadWith" + videoQuality + ".png";
		}
		if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry.png", 10)) {
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png", -400);
		} else if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry_2.png", 10)) {
			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry_2.png", -400);
		}
...
//		} else if (waitForObjectPresent("pictures\\Website_youtube_Button_Registry_2.png", 10)) {
//			moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry_2.png", -300);
//		}
		// moveMouseDownFromLogo("pictures\\Website_youtube_Button_Registry.png", -300);
...
//		sleep(1);
		...
		// }
		//
		// clickOnRegion("pictures\\Browser_Savior_FileType_mp4.png",
		// "pictures\\"+imageName, 150, 120);
		// sleep(1);

		if (waitForObjectPresent("pictures\\" + imageName, 20)) {
			clickOn("pictures\\" + imageName);
		} else {
			TestLogger.info("not found image videoQuality");
		}
	...
	/**
	 * open CocCoc and Access to URL on CocCoc
	 * 
	 * @author HANV
	 */
	...
	public boolean checkFileDownloadWell(String fileQuality) {
		...
		if (meediaPath.exists()) {
			...
			case "FullHD":
				...
				TestLogger.info("FullHD filesize: " + fileSize);
				break;
			case "Small":
	...
	/**
	 * kiểm tra nếu file tồn tại trong ổ đĩa, xóa trước khi download 1 file cùng
	 * tên.
	 * 
	 * @author Nhung.Nguyen
	 * @date 30/11/2017
	 */
	public void DeleteFileInDirectory(String fileName) {
		String mediaFilePath = System.getenv("USERPROFILE") + "\\Downloads\\" + fileName;
		File meediaPath = new File(mediaFilePath);
		if (meediaPath.exists()) {
			meediaPath.delete();
